---
name: plan-feature
description: Plan a feature end-to-end by selecting and sequencing the right planning skills, from discovery through local PRD, spec, agent issue drafts, and human-facing Jira/Confluence docs. Use when the user wants to plan a feature, turn an idea into implementation work, or asks what planning skills/order to use.
---
...
For non-trivial planning sessions, load `agent-breadcrumbs` at the start and maintain `.agent/state/breadcrumbs/<agent-id>.md` through grilling, artifact creation, subagent orchestration, reviews, and verification checkpoints. Update it before long subagent calls and after each completed artifact so `connection-recovery` can reconstruct the plan if the connection drops.
...
- If the user already has an approved spec, run both `to-issues` and `to-jira` unless the user explicitly asks to skip one.
- If the user asks only to stress-test an idea, use `grill-with-docs` and stop.
- If the repo has no useful docs and the user only wants a conversational grilling session, use `grill-me` instead of `grill-with-docs`.
...
After grilling and any prototype checkpoint, use delegated artifact creation by default. Load `cavecrew` before spawning creator and review subagents when available, and tell each creator subagent to use cavecrew for its own nested codebase/file investigation and review helpers where useful.
...
After PRD and spec checkpoints, summarize what exists locally and ask whether to continue. After the spec is approved, create both agent issue drafts and human-facing Jira/Confluence docs before the final summary. Do not publish remotely unless explicitly asked.
